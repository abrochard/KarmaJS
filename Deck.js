var Deck = function(x, y, maxRender) {
  ...
  self.generate = function(faceUp) {
    ...
    for (var suit in SUIT) {
      for (var value = 2; value < FACE.ACE; value++) {
        self.cards.push(
          new Card(value, SUIT[suit], self.x, self.y, faceUp, false)
        );
      }
    }
  };
  ...
  self.render = function(ctx) {
    var count = 0;
    var max = Math.max(0, self.cards.length - maxRender);
    for (var i = max; i < self.cards.length; i++) {
  ...
  self.sameLastFour = function() {
    ...
    for (var i = 1; i < total; i++) {
      ...
      if (self.cards[index].value != SPECIAL.INVISIBLE &&
